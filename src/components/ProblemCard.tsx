      ...
      <div className="p-4 md:p-12 flex-1 flex flex-col">
        ...
        {problem.category === '15 Puzzle' ? (
          <div className="flex-1 flex flex-col">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6 mb-8 bg-slate-50 p-5 rounded-3xl border-[3px] border-[#2D3436] shrink-0">
              <div className="space-y-2 text-center sm:text-left w-full sm:w-auto">
                <span className="block text-[10px] font-black text-[#2D3436]/40 uppercase tracking-[0.2em] italic font-mono">
                ...
                <div className="flex gap-2.5 justify-center sm:justify-start">
                  {[3, 4, 5].map((s) => (
                    <button
                      ...
                      className={`px-4 py-2.5 rounded-xl text-xs font-black border-2 border-[#2D3436] shadow-neo-sm transition-all hover:-translate-y-0.5 active:translate-y-0 cursor-pointer ${
                        gridSize === s 
              ...
              <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto border-t sm:border-t-0 border-[#2D3436]/10 pt-4 sm:pt-0 shrink-0">
                <div className="text-left sm:text-right">
                  <span className="block text-[10px] font-black text-[#2D3436]/40 uppercase tracking-[0.2em] italic font-mono leading-none mb-1">
                    {lang === 'pt' ? 'Movimentos' : 'Moves'}
                  </span>
                  <span className="text-3xl font-black italic tracking-tighter text-[#2D3436] font-mono leading-none">
                    {moveCount}
                  </span>
                </div>
                ...
                <button
                  type="button"
                  onClick={handleReset}
                  className="px-5 py-3.5 bg-neo-yellow hover:bg-[#FFEAA7] text-[#2D3436] border-[3px] border-[#2D3436] rounded-2xl font-black text-xs uppercase tracking-wider shadow-neo-sm hover:shadow-neo transition-all cursor-pointer"
                >
            ...
            <div className="flex-1 flex items-center justify-center p-2 min-h-[300px]">
              <div 
                className="grid p-4 bg-[#2D3436] border-[6px] border-[#2D3436] rounded-[40px] shadow-neo max-w-sm w-full aspect-square gap-2.5 relative"
                style={{
                ...
                {tiles.map((tile, idx) => {
                  ...
                  if (tile === null) {
                    return (
                      <div 
                        key={`empty-${idx}`} 
                        className="bg-[#2D3436]/70 rounded-[18px] border-2 border-dashed border-white/10"
                      />
                    );
                  }
                  ...
                  return (
                    <motion.button
                      ...
                      className={`w-full h-full rounded-[18px] border-[3px] border-[#2D3436] flex items-center justify-center font-display font-black shadow-neo-sm transition-all duration-150 relative cursor-pointer select-none ${
                        isCorrectPos 
                          ? 'bg-neo-green text-white' 
                          : 'bg-white text-[#2D3436] hover:bg-slate-50'
                      }`}
                    >
                      <span className="text-xl md:text-2xl font-black tracking-tighter leading-none italic">
                        {tile}
                      </span>
                      {isCorrectPos && (
